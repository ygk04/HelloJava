...
public class ExCarTester3 {
    public static void main(String[] args) {
        ...
        // 기본 자료형 형변환, 범위 : 좁음 -> 넓음(업 캐스팅), 넓음 -> 좁음(다운 캐스팅)
        // 메모리 크기 (바이트)와는 상관이 없음
        int a = 1;
        // float b = (int ) a; // 컴파일러가 자동 추가해줌
        float b = a; //업캐스팅 (자동 형변환)

        float c = 1.1f;
        int d = (int) c; // 다운 캐스팅 (명시적 형변환), 오버 또는 언플로우 가능성
        ...
        // 명시적 형 변환, 수동 형변환: 상위 참조변수 (Car)에서 하위 참조변수 (ExCar)로 캐스팅
        // 다운 캐스팅
        // car4의 실제 인스턴스는 Car, Car (상위) -> ExCar(하위) 캐스팅
        ...
        // 컴파일러 에러가 발생하지 않음...
        // 강제 형 변환(다운 캐스팅)은 컴파일 단계에서 가능하지만, 실행 시점에 실제 인스턴스 타입에 따라 예외 발생
