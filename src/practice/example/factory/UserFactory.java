...
public class UserFactory {
    // User 객체 생성 : 유일성 확보 힘듬
    // 이름은 동명이인이 있을 수 있으나
    // 이름과 핸드폰 번호의 조합은 유일해야함
    // 해당 객채를 생성하기 전에 이미 생성된 객체를 활용하는 방법 필요
    ...
    // 미리 더미 데이터 생성
    static {
        cache.add(new User("유근규", "010-0000-0000"));
        cache.add(new User("윤하겸", "010-0000-0001"));
        cache.add(new User("박세빈", "010-0000-0002"));
    ...
    public static User from(String name, String phoneNumber) {
        ...
        int i; // 카운터 변수, for문이 수행 종료될 때 i 는 마지막 사용자 idx 를 가짐

            if (cache.contains(newUser)) {
                return cache.get(cache.indexOf(newUser));
             }else {
        // i 번째 (저장해야할 빈공간)에 저장할 수 있음
